---
name: post-process
description: Use when 후처리마스터 탭/영역의 수정·조회·기능추가 요청을 받았을 때. 트리거 — "후처리마스터", "후처리 상품마스터", "post-process", "/post-process", "상품등급", "상품마스터 워크북", "마스터시트", "샘플집계", "증가주문", "차감주문", "post_process_products"
---
...
절 번호는 고정 ID(다른 스킬이 인용). 경로 기준 `.claude/skills/post-process/`.
